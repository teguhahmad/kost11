...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const [roomsData, tenantsData, paymentsData, maintenanceData, currentMonthPayments, lastMonthPayments] = await Promise.all([
        supabase
          .from('rooms')
          .select('*')
      ...
      const currentMonthTotal = (currentMonthPayments.data || [])
        .reduce((sum, payment) => sum + (payment.amount || 0), 0);
      const lastMonthTotal = (lastMonthPayments.data || [])
        .reduce((sum, payment) => sum + (payment.amount || 0), 0);
